...
CollisionAvoidanceSystem::CollisionAvoidanceSystem(const std::string &node_name)
  ...
  for (size_t i = 0; i < obstacle_positions_flat.size(); i += 3)
  {
    std::vector<double> obstacle;
    obstacle.push_back(obstacle_positions_flat[i]);
    obstacle.push_back(obstacle_positions_flat[i + 1]);
    obstacle.push_back(obstacle_positions_flat[i + 2]);
    this->obstacle_positions.push_back(obstacle);
    RCLCPP_ERROR(this->get_logger(),"%f %f %f", obstacle_positions_flat[i], obstacle_positions_flat[i+1], obstacle_positions_flat[i+2]);
  }
  sim = new RVO::RVOSimulator(this->obstacle_positions);
  ...
  */
  ...
  this->goals = std::vector<RVO::Vector3>();
  this->setupScenario();
}
...
void CollisionAvoidanceSystem::vehicle_local_pos_callback(const px4_msgs::msg::VehicleLocalPosition::SharedPtr msg, int robot_id) {
    ...
    RVO::Vector3 new_position = RVO::Vector3(msg->y + this->robot_initial_positions[robot_id-1][0], msg->x + this->robot_initial_positions[robot_id-1][1], -msg->z);
    this->set_agent_position(new_position, robot_id-1);
    RVO::Vector3 new_velocity = RVO::Vector3(msg->vy, msg->vx, -msg->vz);
    this->set_agent_velocity(new_velocity, robot_id-1);
    this->calculate_next_positions();
    // std::vector<RVO::Vector3> agent_positions = this->get_agent_positions();
    ...
    trajectory_setpoint.velocity[2] = -this->sim->getAgentVelocity(this->agentID-1).z();
...
void CollisionAvoidanceSystem::set_initial_positions_from_yaml(){
  for(auto drone_position: this->robot_initial_positions){
    sim->addAgent(RVO::Vector3(drone_position[0], drone_position[1], 5.0));
  }
}
  ...
  if (input_file.is_open()) {
    ...
    while (std::getline(input_file, line)) {
      ...
      if (line_stream >> x >> delimiter >> y) {
        sim->addAgent(RVO::Vector3(x, y, 5.0));
      } else {
...
void CollisionAvoidanceSystem::set_robot_goal_positions_from_yaml() {
  for(auto drone_goal: this->goal_positions){
    this->goals.push_back(RVO::Vector3(drone_goal[0], drone_goal[1], 5.0));
  }
}
...
void CollisionAvoidanceSystem::set_robot_goal_positions(std::string file_path) {
  ...
  if (input_file.is_open()) {
    ...
    while (std::getline(input_file, line)) {
      ...
      // std::shared_ptr<std::vector<Eigen::Vector2d>>
      if (line_stream >> x >> delimiter >> y) {
        this->goals.push_back(RVO::Vector3(x, y, 5.0));
      } else {
        std::cerr << "Error reading line in orca2: " << line << std::endl;
      }
    }
...
void CollisionAvoidanceSystem::setPreferredVelocities() {
  for (std::size_t i = 0U; i < sim->getNumAgents(); ++i) {
    RVO::Vector3 goalVector = this->goals[i] - sim->getAgentPosition(i);
...
void CollisionAvoidanceSystem::set_agent_positions(
    const std::vector<RVO::Vector3> &positions) {
...
std::vector<RVO::Vector3> CollisionAvoidanceSystem::get_agent_positions() {
...
std::vector<RVO::Vector3> CollisionAvoidanceSystem::get_agent_velocities() {
...
void CollisionAvoidanceSystem::calculate_next_positions(
    int attackedRobotId, RVO::Vector3 attackedRobotPosition) {
  updateVisualization();
  setPreferredVelocities();
