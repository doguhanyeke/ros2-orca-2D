...
class CollisionAvoidanceSystem : public rclcpp::Node {
 ...
 public:
  ...
  bool reachedGoal();
  void set_agent_positions(const std::vector<RVO::Vector3> &positions);
  void set_agent_position(const RVO::Vector3 &position, int id);
  void set_agent_velocities(const std::vector<RVO::Vector3> &velocities);
  void set_agent_velocity(const RVO::Vector3 &velocity, int id);

  std::vector<RVO::Vector3> get_agent_positions();
  std::vector<RVO::Vector3> get_agent_velocities();
  void updateVisualization();
  void calculate_next_positions();
  void calculate_next_positions(int attackedRobotId,
                                RVO::Vector3 attackedRobotPosition);
